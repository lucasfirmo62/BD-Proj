...
-- 4- Encontre os nomes e numero de indicacoes dos diretores que tenham mais de 15 indicações e que seus filmes tenham mais de 198 min de duração

select distinct P.nome, D.indicacoes
from diretor D, filme F, titulos T, pessoa P, pessoa_trabalha_titulo PTT
where PTT.id_pessoa = P.id and D.id_pessoa = P.id and D.indicacoes > 15 and F.duracao in (select F.duracao
from filme F
where PTT.id_titulo = T.id and T.id = F.id_titulo and F.duracao > 198);
...
select T.nome 
from titulos T, perfil_assiste_titulo PAT, categoria C
where PAT.id_titulo = T.id
and C.id = T.id_categoria
and not(C.genero = 'TERROR')
group by PAT.id_titulo
having count(PAT.id_titulo) > 2;

-- 8- Encontre pessoas que trabalhram em mais de 2 titulos

select P.nome
from pessoa P, pessoa_trabalha_titulo PTT
...
select PF.nome 
from perfil AS PF, perfil_assiste_titulo AS PAT
...
-- 11- Selecione pessoas que trabalham nos titulos assistidos por @yasmin_bel

select P.nome
from pessoa P, pessoa_trabalha_titulo PTT, titulos T, perfil PF
where P.id = PTT.id_pessoa and T.id = PTT.id_titulo and PF.nome in(select PF.nome
from perfil PF, perfil_assiste_titulo PAT
where PAT.id_titulo = T.id and PAT.nome_perfil = PF.nome and PF.nome = '@yasmin_bel');

-- 12 - Encontre o perfil com idade mais velha que assistiu a sangue negro

select max(PF.idade)
from perfil PF, perfil_assiste_titulo PAT, titulos T
where T.id = PAT.id_titulo and PF.nome = PAT.nome_perfil and T.nome in (select T.nome
from titulos T
where T.nome = 'Sangue Negro');